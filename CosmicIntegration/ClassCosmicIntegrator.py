...
import CosmicIntegration.ClassCOMPAS as ClassCOMPAS
import CosmicIntegration.ClassMSSFR as ClassMSSFR
import CosmicIntegration.selection_effects as selection_effects
import CosmicIntegration.totalMassEvolvedPerZ as MPZ

#Floor's script from https://github.com/FloorBroekgaarden/Double-Compact-Object-Mergers/blob/main/Scripts/ClassCosmicIntegrator.py
...
class CosmicIntegrator(object):
    ...
    def __init__(self,\
                Cosmology=cosmo, hubbleConstant = 67.8, omegaMatter=0.308, redshiftFirstSFR=10., \
                COMPASpath=None,  COMPASlazyData=True, COMPASMlower=5.,\
                COMPASMupper=150.,COMPASbinaryFraction=1.0,COMPASmetallicityGrid=None,\
                minRedshift=0.0,   maxRedshift=2., nrRedshiftBins=100,\
                DCOtypes=None, withinHubbleTime=True, optimistic=False, 
                RedshiftTabulated =True, RedshiftTabulatedResolution=100000,
                GWdetector_sensitivity='design', GWdetector_snrThreshold=8, verbose = False):
        ...
        ################################################################
        #     initialize data COMPAS popsynth                          #
        #                                                              #
        #     creates metallicity grid and total mass evolved          #
        #     other things  have to be set with COMPAS.setCOMPASData   #
        ################################################################
        if self.verbose:
            print("Creating instance COMPAS class")
        self.COMPAS  = ClassCOMPAS.COMPASData(path=COMPASpath, lazyData=COMPASlazyData,\
                                              Mlower=COMPASMlower, Mupper=COMPASMupper, \
                                              binaryFraction=COMPASbinaryFraction)
        if DCOtypes is not None:
            self.COMPAS.setCOMPASDCOmask(types=DCOtypes,\
                                   withinHubbleTime=withinHubbleTime,\
                                   optimistic=optimistic)
            self.COMPAS.setCOMPASData()
        #####################################################
        #     set the MSSFR class                           #
        #     see if we have a grid from COMPAS   else set  #
        # empty class so you can still access functions     #
        #####################################################

        if self.COMPAS.metallicityGrid is not None:
            #create instance with information of simulation
            if self.verbose :
                print()
                print("Creating instance MSSFR class")
                print("  COMPAS path is given hence I set:")
                print("  metallicity Grid from unique metallicities allSystems")
                print("  calculate 'true' masss evolved using pythonSubmit limits given")
            self.MSSFR = ClassMSSFR.MSSFR(metallicityGrid=self.COMPAS.metallicityGrid,\
                                          totalMassEvolvedPerZ  = self.COMPAS.totalMassEvolvedPerZ,\
                                          cosmo=self.cosmology)       
        else:
            if self.verbose:
                print()
                print( "Creating instance MSSFR class")
                print( "  COMPAS data is not given given hence I set nothing")
            #empty instanec of MSSFR, but you can acces the functions
            self.MSSFR = ClassMSSFR.MSSFR(metallicityGrid=None,\
                                          totalMassEvolvedPerZ  = None,\
                                          cosmo=None)   
    ...
    def cosmologicalIntegration(self):
        ...
        for nr in range(int(self.nrRedshiftBins)):
            for nrZ, Z in enumerate(self.COMPAS.metallicityGrid):
                ...
                # print('is this 0?: ')
                # print(len(self.PerSystemPerRedshift_ageBirth[nr][maskZ]))
                # print(len(self.PerSystemPerRedshift_redshiftBirth[nr][maskZ]))
                # print('end')
                # print()
